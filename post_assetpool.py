...
def write_assetpool(source, target, env):
    if not os.path.exists(assetpool_bin):
        print("warning: AssetPool.bin not found; skipping assetpool write")
        print(
            "hint: build it once in the upstream repo: cmake .. && make -j; then copy build/desktop/AssetPool.bin here"
        )
        return
    # pick an upload port (user can also set upload_port in platformio.ini)
    port = env.GetProjectOption("upload_port") or (env.GetSerialPorts() or [None])[0]
    if not port:
        print("warning: no serial port detected; cannot write assetpool")
        return
    cmd = ["parttool.py", "--port", port, "write_partition", "--partition-name=assetpool", "--input", assetpool_bin]
